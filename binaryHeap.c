/*******************************************************************************************
Assignment 2-1 Problem 3
UC Santa Cruz Ext, 2023 Fall - CMPR X401 (820) - Radhika Grover
Description:  
1. Min Binary Tree was modified as Max Binary Tree
2. The function deleteMax has been added

Date              Author                Revision             Notes
11/24/2023     Herbe Chun        v1                       None
*/
...
/*******************************************************************************************
For the Min or Max binary tree 
The difference between the min and max binary tree is the equality statement.
  Min tree uses: heap->arr[i / 2]->distance > item
  Max tree uses: heap->arr[i / 2]->distance < item
*/
void insert(BinaryHeap *heap, int _distance, int _value) {
	...
	*newNode = (Node) {.distance = _distance, .value = _value};
	...
	if (isFull(heap)) {
		...
		while (i > 1 && heap->arr[i / 2]->distance < item) {
...
/*******************************************************************************************
The difference between the deleteMax and deleteMin is the equality statement.
   deleteMin uses:  if (child != heap->size && heap->arr[child + 1]->distance < heap->arr[child]->distance)
   deteleMax uses:  if (child != heap->size && heap->arr[child + 1]->distance > heap->arr[child]->distance)
*/
Node* deleteMax(BinaryHeap *heap) {
    ...
    heap->arr[i] = lastNode;
    return maxNode; // largest element in array
}
...
int main() {
	...
	printf("\n Delete maximum item with distance: %d   value:  %d\n", minKey->distance,  minKey->value);
